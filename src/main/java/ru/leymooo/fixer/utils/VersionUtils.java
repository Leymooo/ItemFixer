...
public class VersionUtils {
    ...
    private static void detectServerVersion() {
        ...
        try {
            ...
            JavaPlugin.getPlugin(Main.class).getLogger().log(Level.WARNING, "Could not detect bukkit version. Trying another method"
                    , e);
            try {
                ...
                JavaPlugin.getPlugin(Main.class).getLogger().log(Level.WARNING, "Could not detect bukkit version. Trying another " +
                        "method", e2);
                try {
                    String[] split = Bukkit.getServer().getClass().getPackage().getName().split("\\.")[3].split("_");
                    majorVersion = Integer.parseInt(split[1]);
                } catch (Exception e3) {
                    JavaPlugin.getPlugin(Main.class).getLogger().log(Level.WARNING, "Could not detect bukkit version. Fallback to 1" +
                            ".8.8.", e3);
                    majorVersion = 8;
                    minorVersion = 8;
                }
