...
public class XListener implements Listener{  
	...
	public void onPickupItem(final PlayerPickupItemEvent e) {
		...
		if (!player.hasPermission("itemfixer.bypass")) {
			...
			final boolean a = ru.xtime_1_8_R1.Checks.checkAttributes(item.getItemStack());
			if (a) {
				e.setCancelled(true);
				item.remove();
			}
			ru.xtime_1_8_R1.Checks.removeEnt(item.getItemStack());
		}
	}
	...
	public void OnLaunch2 (BlockDispenseEvent e){
		...
		final boolean a = ru.xtime_1_8_R1.Checks.checkAttributes(item);
		if (a) {
			e.setCancelled(true);
		}
		final boolean b = ru.xtime_1_8_R1.Checks.removeEnt(item);
		e.setCancelled(b);
	}
	@EventHandler
	public void InventoryClick(final InventoryClickEvent e) {
		...
		if (e.getCurrentItem() != null) {
			...
			if (!player.hasPermission("itemfixer.bypass")) {
				...
				ru.xtime_1_8_R1.Checks.removeEnt(item);
			}
		}
	}
	...
	public void lol(final PlayerInteractEvent e) {
		...
		if(e.getPlayer().hasPermission("itemfixer.bypass")) {
			...
			if (its != null && its.getType() == Material.MONSTER_EGG) {
				final boolean a = ru.xtime_1_8_R1.Checks.checkAttributes(its);
				if (a) {
					this.b = true;
				}else {
					this.b = false;
