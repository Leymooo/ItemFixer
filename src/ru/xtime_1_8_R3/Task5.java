		...
		for (final Player p : Bukkit.getServer().getOnlinePlayers()) {
			if (!p.hasPermission("itemfixer.bypass")) {
				...
				for (ItemStack it : p.getInventory().getContents()) {
					if (it != null) {
						final boolean a = ru.xtime_1_8_R3.Checks.checkAttributes(it);
						if (a) {
							p.getInventory().remove(it);
						}
						ru.xtime_1_8_R3.Checks.removeEnt(it);
					}
				}
				for (ItemStack it2 : p.getInventory().getArmorContents()) {
					if (it2 !=null) {
						final boolean a = ru.xtime_1_8_R3.Checks.checkAttributes(it2);
						if (a) {
							p.getEquipment().setArmorContents(null);
						}
						ru.xtime_1_8_R3.Checks.removeEnt(it2);
					}
				}
			}
		}
